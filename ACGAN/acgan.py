...
def make_generator_model(input_tensor=None,
  ...
  model = models.Model([noise, label], img, name='Generator_model')
  return model
...
def make_discriminator_model(input_tensor=None,
  ...
  validity = layers.Dense(1, activation=tf.nn.sigmoid)(features)
  label = layers.Dense(num_classes, activation=tf.nn.softmax)(features)

  model = models.Model(img, [validity, label])
  return model
...
if __name__ == '__main__':
  train_images, train_labels = load_data(BUFFER_SIZE, BATCH_SIZE)
